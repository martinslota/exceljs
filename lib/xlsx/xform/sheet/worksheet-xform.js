  ...
  reconcile(model, options) {
    ...
    const rels = (model.relationships || []).reduce((h, rel) => {
      ...
      if (rel.Type === RelType.Comments) {
        // The comments part may not have been loaded — exceljs only
        // recognises `xl/commentsN.xml` on read, so a rel pointing at e.g.
        // `../sheet1_comments.xml` leaves no entry in options.comments.
        // Skip rather than dereferencing undefined.
        const commentsPart = options.comments[rel.Target];
        if (commentsPart) {
          model.comments = commentsPart.comments;
        }
      }
    ...
    if (model.drawing) {
      ...
      const match = drawingRel && drawingRel.Target.match(/\/drawings\/([a-zA-Z0-9]+)[.][a-zA-Z]{3,4}$/);
      if (match) {
        const drawingName = match[1];
        const drawing = options.drawings[drawingName];
        // The drawing may be absent or empty when its part used an
        // unrecognised root element (e.g. <c:userShapes>, or <wsDr>
        // without the xdr: prefix). Skip media extraction in that case.
